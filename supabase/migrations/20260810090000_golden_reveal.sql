...
-- Schema-qualified throughout. The function bodies below pin their own
-- search_path, but statements at this level inherit whatever the connection
-- happens to have — and a SQL Editor session that doesn't include public
-- fails with a bare "relation does not exist" that reads like a missing table.
...
CREATE INDEX IF NOT EXISTS saved_words_pending_green_idx
  ON public.saved_words (user_id, language)
  WHERE state = 'green';
 ...
 */
CREATE OR REPLACE FUNCTION public.reveal_green_word(p_word_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_row public.saved_words%ROWTYPE;
BEGIN
...
GRANT EXECUTE ON FUNCTION public.reveal_green_word(uuid) TO authenticated;
 ...
 */
CREATE OR REPLACE FUNCTION public.touch_gold_word(p_word_id uuid, p_decay_at integer DEFAULT 3)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
GRANT EXECUTE ON FUNCTION public.touch_gold_word(uuid, integer) TO authenticated;
